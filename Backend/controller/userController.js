...
export const login = catchAsyncErrors(async (req, res, next) => {
  ...
  if (role !== user.role) {
    return next(
      new ErrorHandler(
        `This account is registered as ${user.role}. Please login from the correct portal.`,
        400
      )
    );
  }
...
export const addNewDoctor = catchAsyncErrors(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorHandler(
        "Cloudinary is not configured. Please set CLOUDINARY_* values in config.env.",
        500
      )
    );
  }
  ...
  try {
    cloudinaryResponse = await cloudinary.uploader.upload(doctrAvatar.tempFilePath);
  } catch (uploadError) {
    return next(
      new ErrorHandler(
